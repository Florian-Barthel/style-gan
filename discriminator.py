...
class Discriminator(tf.keras.models.Model):
    def __init__(self,
        ...
        for res in range(self.resolution_log2, 2, -1):
            self.from_rgb_first[res] = layers.FromRGB(res, fmap_base, use_wscale=use_wscale, gain=gain)
            self.blocks[res] = layers.DiscBlock(res, fmap_base, use_wscale=use_wscale, gain=gain)
            self.from_rgb_downscaled[res] = layers.FromRGB(res - 1, fmap_base, use_wscale=use_wscale, gain=gain)
        self.from_rgb_first[2] = layers.FromRGB(2, fmap_base, use_wscale=use_wscale, gain=gain)
